...
%syms agrega la variable a la ecuaci�n
syms t

%polyfit devuelve los coeficientes para un polinomio (ajusta a m�nimos
%cuadrados)
ec = polyfit(x,y,1)

%Se divide entre 1*10^3 para deshacer la factorizaci�n que realiza MATLAB
%por default
lEc = (ec./(1.0.*10^(3)))

%Se crea una expresi�n polinomial para un vector simb�lico de coeficientes
%A partir del polinomio anterior se crea la ecuaci�n de la recta.
llEc = poly2sym(ec,t)

%children encuentra las subexpresiones o t�rminos de una expresi�n simb�lica
sp = children(llEc)

%vpa devuelve una aproximaci�n num�rica para expresiones  que utilizan
%variables simb�licas
t1 = vpa(sp(1))
t2 = vpa(sp(2))


%Se pasan por separado los t�rminos del polinomio correspondiente a la
%ecuaci�n y se convierten a caracteres los t�rminos de la ecuaci�n, 
%debido a que �stas son de tipo variable simb�lica.
set(handles.ecuacionE,'string',char(t1))
set(handles.ecuacionE2, 'string',char(t2))

%Se ajusta el rango de tendencia de los valores en x para la l�nea
xFitted = 0:19
